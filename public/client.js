...
if (onClickCopyElements.length > 0) {
    onClickCopyElements.forEach(onClickCopyElement => {
        onClickCopyElement.addEventListener('click', (e) => {
            ...
            navigator.clipboard.writeText(copyText).catch(error => console.error('Error when copying to clipboard', error))
        })
    })
}

const submitConfirmElements = document.querySelectorAll('[submit-delete-confirm]')
...
const newPasswordConfirmedInput = document.getElementById('newPasswordConfirmed')
...
if (newPasswordInput && newPasswordConfirmedInput) {
    [
        ...
        input.addEventListener('change', () => {
            const newPasswordConfirmedInput = document.getElementById('newPasswordConfirmed')
            if (newPasswordConfirmedInput.value !== document.getElementById('newPassword').value) {
